...
SELECT content_type,ROUND(SUM(likes+comments+shares)/3,2) AS Average_engagement FROM posts
WHERE content_type IN ('video','text','image')
GROUP BY content_type
ORDER BY Average_engagement 
LIMIT 1;
...
SELECT HOUR(post_time) AS Hour_of_posting, ROUND(SUM(likes+comments+shares)/3,2) AS Average_engagement FROM posts
...
SELECT campaign_name, AVG(likes)/COUNT(*) AS High_Avg_Likes_per_Post FROM Posts
...
-- 9. Which campaigns have above-average comment engagement?(WRONG)
...
SELECT 
	CASE WHEN ROUND(video_length_sec/60) <=2 THEN 'Short video'
		ELSE 'Long video'
...
SELECT content_type, comments/likes AS comment_to_like_ratio FROM posts
...
SELECT DAYNAME(post_date) AS Day_of_Week, 
    ...
    SUM(comments+likes+shares) AS Total_Engagements FROM posts
GROUP BY Day_of_Week;
SELECT HOUR(post_time) AS Hour_of_Day,
	COUNT(*) AS Total_Posts, SUM( shares) AS Total_Shares,
    SUM(comments) AS Total_Comments,SUM(likes) AS Total_Likes, 
    SUM(comments+likes+shares) AS Total_Engagements FROM posts
